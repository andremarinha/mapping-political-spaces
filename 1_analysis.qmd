...
# Scree plots with Benzécri correction
eig_plots <- list()
for (cc in countries) {
...
(mca_eig_plots[["GR"]] + mca_eig_plots[["IT"]]) +
  plot_annotation(title = "Pooled MCA Scree Plots (All Rounds Stacked)")
```
...
for (cc in countries) {
  ...
  mca_cat_plots[[cc]] <- ggplot(coord, aes(`Dim 1`, `Dim 2`,
    ...
    scale_colour_viridis_d(option = "plasma", end = 0.85) +
    labs(title = country_labels[cc],
         x = paste0("Dim 1 (", round(mca_results[[cc]]$eig[1, 2], 1), "%)"),
         y = paste0("Dim 2 (", round(mca_results[[cc]]$eig[2, 2], 1), "%)"),
         colour = "Variable") +
    theme(plot.title = element_text(face = "bold", size = 11),
          legend.position = "bottom")
}
...
save(
  df, mfa_results, mfa_inputs, mca_results,
