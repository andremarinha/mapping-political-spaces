...
## Session 6: Social Space MCA (2026-02-24)

### Actions Taken

1. **Discussed political space variable count concern**
   - 5 active variables (3 immigration), Q=19, J=5 → threshold=0.2
   - Benzécri 2-dim solution partly reflects low variable count, not necessarily rich structure
   - 3 immigration items may essentially measure one construct → 3 constructs coded as 5 variables
   - Caveat documented; category configurations and supplementary projections remain valid

2. **Audited social space complete-case coverage**
   - 6 active variables: oesch8, income_quint_h, eisced_5cat, mother_edu_5cat, father_edu_5cat, domicil_r
   - J=6, Q=32 categories, 26 dimensions
   - Complete cases: 38.2% (income_quint_h) / 34.3% (income_quint)
   - Per country: ES=11,295, PT=6,074, IT=5,572, GR=2,765 — all adequate for MCA
   - GR only has R5+R11 with complete data

3. **Implemented social space MCA in `1_analysis.qmd`**
   - New "Social Space Construction" section with ~10 chunks
   - Active: oesch8 (8), income_quint_h (5), eisced_5cat (5), mother_edu_5cat (5), father_edu_5cat (5), domicil_r (4)
   - Supplementary: essround, vote_d (Did not vote / Voted), lca_class
   - Labelled factors for all variables (oesch8_labels, income_labels, edu_labels, etc.)
   - Benzécri correction: Dim 1 = 75–88% modified, Dim 2 = 8–15% modified across countries
   - Full category configuration maps (all 32 categories) coloured by variable
   - **Focused Oesch-8 map**: class positions as large bold points + LCA repertoires (triangles) + vote/abstention (squares)
   - Variable contributions (top 15 for Dim 1 and Dim 2)
   - Robustness section with income_quint (R4–11 only): nearly identical eigenvalue structure

4. **Fixed FactoMineR category naming issue**
   - FactoMineR adds variable name prefix when labels are shared (education vars share "ISCED 0-1" etc.)
   - Unique labels (oesch8, income, domicil) keep their label name without prefix
   - Category-to-variable matching now uses known factor level values, not prefix parsing

5. **Portugal test results (Dim 1 = capital volume)**
   - Positive: ISCED 6+, Q5, managers, socio-cultural professionals, self-emp professionals
   - Negative: ISCED 0-1, Q1, production workers, rural
   - "Did not vote" supplementary: falls on low-capital side (−0.18 on Dim 1)
   - "Voted": slightly positive on Dim 1 (0.05)

6. **Updated Save Workspace** to include social space objects

### Decisions

- Oesch-8 is ACTIVE in the social space (not supplementary) — D26
- Both income variables kept for robustness — D24 (extended)
- Party families (CHES / Party Manifesto) to be added as supplementary once user provides harmonisation — D27

### Current Project State

- `1_analysis.qmd` complete: LCA + political space (pooled MCA + MFA) + social space (pooled MCA + robustness) + Benzécri corrections
- Full QMD not yet rendered
- Data pipeline: 8 scripts (01–08), all functional
- Master: 67,358 × 1,702
...
- User to provide party family harmonisation (CHES / Party Manifesto)
- Render full QMD and inspect all outputs
- Interpret both spaces substantively
- Label LCA classes with meaningful names
- Add diachronic trajectories (round×class barycentres)
